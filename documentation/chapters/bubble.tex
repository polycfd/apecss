...
& {\tt MinTimeStep <float>} & Minimum allowable time-step $\Delta t$.\\ 
& {\tt MaxTimeStep <float>} & Maximum allowable time-step $\Delta t$.\\ 
...
In APECSS, every bubble needs to contain a gas, which requires to select an appropriate equation of state and define meaningful properties.
...
In the same way that every bubble needs a to contain a gas, in APECSS every bubble needs to be surrounded by a liquid, which requires to select an appropriate equation of state and fluid type, as well as define meaningful properties.
