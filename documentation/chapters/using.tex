...
Installing APECSS on Windows is a little more involved than the installation on Unix. The installation\footnote{We are grateful to Maxime Montois for these detailed installation instructions.} described below presents a minimalist installation of APECSS on Windows 11, using {\tt MinGW} and {\tt MSYS2}. 
...
\begin{enumerate}[noitemsep]
  ...
  pacman -S mingw-w64-x86_64-make\end{lstlisting}
   Enter {\tt Y} when prompted to proceed with the installation. To check the installation, you can run these commands:
\begin{lstlisting}[style=CStyle,numbers=none]
  gcc --version
  cmake --version
  make --version\end{lstlisting}
