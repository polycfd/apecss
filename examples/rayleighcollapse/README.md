...
Using [emissions.apecss](emissions.apecss) with ````./build/rayleigh_apecss -options emissions.apecss -tend 0.2```` runs a Rayleigh collapse of a bubble and records its acoustic emissions at different locations in space. The Noble-Abel (NA) equation-of-state is used to describe the gas and the Tait equation-of-state is to describe the liquid. The emissions are modelled based on the Kirkwood-Bethe hypothesis.
