...
APECSS_FLOAT _tauL_, _Rnbd_, _Rnc1_, _Rnc2_, _tmax1_, _tmax2_;
...
APECSS_FLOAT lic_particlevelocityderivative(APECSS_FLOAT *Sol, APECSS_FLOAT t, struct APECSS_Bubble *Bubble)
{
  ...
  else
  {
    ...
    APECSS_FLOAT dotP = ((2.0 * pinf * Rn + 3.0 * sigma) / (3.0 * APECSS_POW4(Bubble->R0) * _tauL_)) * (APECSS_POW3(_Rnbd_) - APECSS_POW3(Bubble->R0)) *
                        (1.0 - APECSS_COS(APECSS_PI * t / _tauL_));  // Eq (3.22) in Liang et al., JFM 940 (2022), A5
...
APECSS_FLOAT lic_equilibriumradius(APECSS_FLOAT t, struct APECSS_Bubble *Bubble)
{
  if (t < 2.0 * _tauL_)  // Eq. (3.9) in Liang et al., JFM 940 (2022), A5
    return (APECSS_POW((APECSS_POW3(Bubble->R0) +
                        (APECSS_POW3(_Rnbd_) - APECSS_POW3(Bubble->R0)) * (t - (_tauL_ / APECSS_PI) * APECSS_SIN(APECSS_PI * t / _tauL_)) / (2.0 * _tauL_)),
                       APECSS_ONETHIRD));
  else if (t < _tmax1_)
    return (_Rnbd_);
  else if (t < _tmax2_)
...
APECSS_FLOAT lic_gas_pressure_hc(APECSS_FLOAT *Sol, struct APECSS_Bubble *Bubble)
{
  ...
  if (Bubble->t > _tmax1_) rhc = Rn / 9.0;
...
APECSS_FLOAT lic_gas_pressurederivative_hc(APECSS_FLOAT *Sol, APECSS_FLOAT t, struct APECSS_Bubble *Bubble)
{
  ...
  // Definition of the hardcore radius based on the discussion in the last paragraph of page 15 in Liang et al., JFM 940 (2022), A5
  APECSS_FLOAT rhc = 0.0;
  if (Bubble->t > _tmax1_) rhc = lic_equilibriumradius(Bubble->t, Bubble) / 9.0;
